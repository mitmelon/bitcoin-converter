...
use Watermelon\BitcoinConverter\Exception\UnexpectedValueException;
...
abstract class AbstractProvider implements ProviderInterface
{
    ...
    public function __construct(Client $client = null, CacheInterface $cache = null, $cacheTTL = 60)
    {
        ...
        if (is_null($cache)) {
            $cache = new Repository(new FileStore(new Filesystem, $_SERVER['DOCUMENT_ROOT']));
        }
    ...
    {
        if (! $this->is_currency_code($currencyCode)) {
     ...
     * @return boolean
     */
    public function is_currency_code($currencyCode)
    {
     ...
     * @return float
     */
    public function to_currency($currencyCode, $btcAmount, $provider = null)
    {
     ...
     * @return float
     */
    public function to_btc($amount, $currencyCode, $provider = null)
    {
